...
export default function LocalList({ title, type, data, onDetailClick, onCheckChange, selectedData }: ILocalList) {
            ...
            {data.map(item => (
              <li key={key(item)} className="flex items-center gap-[5px]">
                <input
                  ...
                  value={item.name}
                  checked={selectedData?.map(data => `${data.depth}#${data.code}`)?.includes(key(item))}
