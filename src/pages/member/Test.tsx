...
function Test() {
  ...
  const handleDistrictDetailClick = (item: LocalData) => {
    const code = item.code;
    const district = districtSectionData.find(data => data.code === code);
  ...
  const handleCheckChange = (item: LocalData, isChecked: boolean) => {
    ...
    if (isChecked) {
      ...
      if (hasChild(item)) {
        if (item?.children) {
          setSelectedData(prev => [...prev, item?.children[0]]);
        }
      }
      /* 나와 같은 뎁스, 같은부모에 선택된 is_whole이 있으면 제외 */
      const isWhole = selectedData.some(data => data.depth === item.depth && data.parent_code === item.parent_code && data.is_whole);
      if (selectedData.some(data => isWhole)) {
  ...
  const isRepresentativeByChild = (item): boolean => {
    return hasChild(item) && selectedData && selectedData.some(data => data.depth === item.depth + 1 && data.parent_code === item.code && data.is_whole);
  }

  /*부모 가져오기*/
  const getParent = (item: LocalData): LocalData | undefined => {
    return selectedData && selectedData.find(data => data.depth === item.depth - 1 && data.code === item.parent_code);
  }
...
// 선택 지역 리스트 - 상위 지역 정보 찾기
  const getFullLocationName = (item: LocalData): string => {
    ...
    switch (item.depth) {
      ...
      case 2:
        return isRepresentativeByChild(item) ? '' : item.is_whole ? item.name : `(${city}) ${item.name} 2`;
      case 3:
