...
import LocalList2, { LocalData } from './TestList';

export default function LocalPlus() {
  ...
  function hasChild(item: LocalData) {
      ...
      if (hasChild(item)) {
        if (item?.children && item?.children?.length > 0) {
          setSelectedData(prev => {
            const child = item?.children?.find(data => data.is_whole);
            return child ? [...prev, child] : prev;
          });
        }
      }
   ...
   * */
  const isRepresentativeByChild = (item: any) => {
    return (
      hasChild(item) &&
      selectedData &&
